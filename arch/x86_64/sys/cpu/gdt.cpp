...
namespace AEX::Sys {
    ...
    void mcore_gdt(tss** tsses) {
        ...
        gdt[0].setBase(0).setLimit(0).setPrivilege(0);
        gdt[0].access = AC_NONE;
        gdt[0].flags  = FL_NONE;

        gdt[1].setBase(0).setLimit(0xFFFFF).setPrivilege(0);
        gdt[1].access |= AC_EXECUTABLE | AC_READ_WRITE | AC_CODE_DATA | AC_PRESENT;
        gdt[1].flags = FL_GRANULARITY | FL_X64;

        gdt[2].setBase(0).setLimit(0xFFFFF).setPrivilege(0);
        gdt[2].access |= AC_READ_WRITE | AC_CODE_DATA | AC_PRESENT;
        gdt[2].flags = FL_NONE;

        gdt[3].setBase(0).setLimit(0xFFFFF).setPrivilege(3);
        gdt[3].access |= AC_READ_WRITE | AC_CODE_DATA | AC_PRESENT;
        gdt[3].flags = FL_NONE;

        gdt[4].setBase(0).setLimit(0xFFFFF).setPrivilege(3);
        gdt[4].access |= AC_EXECUTABLE | AC_READ_WRITE | AC_CODE_DATA | AC_PRESENT;
        ...
        for (int i = 0; i < MCore::cpu_count * 2; i += 2) {
            ...
            gdt[5 + i].setLimit(sizeof(tss)).setBase(tss_addr).setPrivilege(3);
            gdt[5 + i].access |= AC_EXECUTABLE | AC_ACCESSED | AC_PRESENT;
